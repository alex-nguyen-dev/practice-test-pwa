        ...
        {NAV_ITEMS.map(({ label, icon: Icon, path }) => {
          ...
          return (
            <button
              key={path}
              onClick={() => navTo(path)}
              className={`flex flex-1 flex-col items-center gap-0.5 rounded-2xl px-2 pb-0 pt-2 text-[11px] font-semibold transition active:scale-95 ${active ? 'text-sky-600 dark:text-sky-300' : 'text-slate-500 dark:text-slate-400'}`}
            >
